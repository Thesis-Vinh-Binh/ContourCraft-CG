...
from .lbs import lbs, hybrik, rotmat_to_quat, quat_to_rotmat, get_rest_pose_lbs
...
class SMPL_layer(nn.Module):
    ...
    def get_rest_pose(self, beta, transl):
        rest_J, v_shaped = get_rest_pose_lbs(beta, self.v_template, self.shapedirs, self.J_regressor)

        if transl is not None:
            rest_J += transl.unsqueeze(dim=1)
            v_shaped += transl.unsqueeze(dim=1)
        else:
            v_shaped = v_shaped - rest_J[:, self.root_idx_smpl, :].unsqueeze(1).detach()
            rest_J = rest_J - rest_J[:, self.root_idx_smpl, :].unsqueeze(1).detach()
