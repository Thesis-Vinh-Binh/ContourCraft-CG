...
class SMPL(nn.Module):
    ...
    def forward(
        ...
        **kwargs
        ...
        lbs_out = lbs(betas, full_pose, self.v_template,
                               self.shapedirs, self.posedirs,
                               self.J_regressor, self.parents,
                               self.lbs_weights, pose2rot=pose2rot)

        vertices, joints, rot_mats, A = lbs_out
        T = None
...
class SMPLH(SMPL):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # print('left_hand_mean', left_hand_mean, flat_hand_mean)
        ...
        self.register_buffer('pose_mean', pose_mean_tensor.contiguous())
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.register_buffer('lmk_faces_idx',
                             torch.tensor(lmk_faces_idx, dtype=torch.long).contiguous())
        lmk_bary_coords = data_struct.lmk_bary_coords
        self.register_buffer('lmk_bary_coords',
                             torch.tensor(lmk_bary_coords, dtype=dtype).contiguous())
        ...
        # print('self._num_expression_coeffs', self._num_expression_coeffs, expr_start_idx, expr_end_idx)
    ...
    def forward(
        ...
        **kwargs
        ...
        global_orient = (global_orient if global_orient is not None else
                         self.global_orient)
        body_pose = body_pose if body_pose is not None else self.body_pose
        # print('self.body_pose', self.body_pose.shape)
        betas = betas if betas is not None else self.betas
        ...
        # print('right_hand_pose', right_hand_pose, left_hand_pose)
...
class SMPLXLayer(SMPLX):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # print(extended_parents, children_map)

        if 'default_params' in kwargs:
            with open(kwargs['default_params'], 'rb') as f:
                default_params = pk.load(f)

            default_v = default_params['vertices'].detach().cpu().float()
            joints_full = default_params['joints'].detach().cpu().float()
            leaf_vertices = default_v[:, self.LEAF_INDICES].clone()
            joints_default = torch.cat([joints_full[:, :55], leaf_vertices], dim=1)

            self.register_buffer('default_vertices', default_v)
            self.register_buffer('default_joints', joints_default)
            # print(default_params['joints'].shape, extended_parents.shape)
    ...
    def forward(
        ...
        **kwargs
        ...
        vertices, joints, _, A = lbs(shape_components, full_pose, self.v_template,
                               shapedirs, self.posedirs,
                               self.J_regressor, self.parents,
                               self.lbs_weights,
                               pose2rot=False,
        ...
        leaf_vertices = vertices[:, self.LEAF_INDICES].clone()
        joints_hybrik = torch.cat([joints[:, :55], leaf_vertices], dim=1)
        ...
        if transl is not None:
            ...
            joints -= root_j
            vertices -= root_j
            joints_hybrik -= root_j

        output = edict(vertices=vertices if return_verts else None,
                             joints=joints,
                             betas=betas,
                             expression=expression,
                             global_orient=global_orient,
    ...
    def forward_simple(
        ...
        **kwargs
        ...
        if expression is None:
            expression = torch.zeros(betas.shape[0], self._num_expression_coeffs, device=betas.device)
        ...
        lbs_out = lbs(shape_components, full_pose, self.v_template,
                               shapedirs, self.posedirs,
                               self.J_regressor, self.parents,
                               self.lbs_weights,
                               pose2rot=pose2rot,
        ...
        T = None
        if return_transform:
            vertices, joints, rot_mats, A, T = lbs_out
        else:
            vertices, joints, rot_mats, A = lbs_out
        
        if scale is not None:
            root_j = joints[:, 0, :].clone()
            vertices = vertices - root_j.unsqueeze(dim=1)
            joints = joints - root_j.unsqueeze(dim=1)

            vertices = vertices * scale.unsqueeze(dim=1)
            joints = joints * scale.unsqueeze(dim=1)

            # A: batch x N x 4 x 4
            assert len(A.shape) == 4
            A[:, :, :3, 3] = A[:, :, :3, 3] - root_j.unsqueeze(dim=1)
            A[:, :, :3] = A[:, :, :3] * scale.reshape(1)
            # print(A[:, 0], root_j, scale)

        # print('joints', joints.shape, transl.shape)
        if transl is not None:
            joints = joints + transl.unsqueeze(dim=1)
            vertices = vertices + transl.unsqueeze(dim=1)
            # A[:, :, :3, 3] += transl.unsqueeze(dim=1)
            
        elif root_align: ################################ elif !!!!!!!!!
            root_j = joints[:, [0], :].clone()
            joints -= root_j
            vertices -= root_j
            # A[:, :, :3, 3] -= root_j.unsqueeze(dim=1)
    ...
    def get_rest_pose(self, beta, align=True):
        vertices, joints = lbs_restpose(beta, self.v_template,
                               self.shapedirs,
                               self.J_regressor)
        ...
        return edict(vertices=vertices, joints=joints)

    def get_disp(self, diff_betas, pose_old, pose_new):
        batch_size = diff_betas.shape[0]
        device = diff_betas.device
        blend_shape = torch.einsum('bl,mkl->bmk', [diff_betas, self.shapedirs])
        
        rot_mats_old = batch_rodrigues(pose_old.view(-1, 3)).view(
            [batch_size, -1, 3, 3])

        rot_mats_new = batch_rodrigues(pose_new.view(-1, 3)).view(
            [batch_size, -1, 3, 3])

        ident = torch.eye(3, device=device)
        pose_feature = (ident - rot_mats_old[:, 1:, :, :]).view([batch_size, -1])

        pose_offsets = torch.matmul(
            pose_feature, self.posedirs).view(batch_size, -1, 3)
        
        vertices_disp = blend_shape + pose_offsets
        joints_disp = torch.einsum('bik,ji->bjk', [vertices_disp, self.J_regressor])
        return vertices_disp, joints_disp
