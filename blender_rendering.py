...
parser.add_argument("--obj_dir", type=str, required=True, help="path to the save resules shapenet dataset")
...
random.seed(args0.seed + args0.frameid * 1345 + 17532)
HOOD_PROJECT = os.path.dirname(__file__)
    
saved_folder_final = os.path.join(HOOD_PROJECT, 'exp/example_simulation/motion_0')
obj_dir = os.path.join(saved_folder_final, f'meshes/')
objs = [item for item in os.listdir(obj_dir) if item.endswith('.obj')]

frameid = args0.frameid

suffix = '.obj'
body_path = os.path.join(
    saved_folder_final, f'meshes/SMPLX-mesh-neutral_{frameid}{suffix}', 
)

garment_path1 = os.path.join(
saved_folder_final, f'meshes/lower_{frameid}{suffix}',
)

garment_path2 = os.path.join(
    saved_folder_final, f'meshes/upper_{frameid}{suffix}',
)

garment_txt_path1 = 'assets/single_color_texture_0.png'
garment_txt_path2 = 'assets/single_color_texture_1.png'
body_txt_path = 'assets/smplx_texture_m_2023.png'

output_folder = os.path.join(saved_folder_final, f'imgs/{args0.frameid}')
